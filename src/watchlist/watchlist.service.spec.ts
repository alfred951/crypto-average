...
describe('WatchlistService', () => {
  ...
  const symbolList: CryptoSymbol[] = [
    { id: null, symbol: testSymbol, lectures: [] },
  ];

  beforeEach(async () => {
    const repositoryMockFactory: () => MockType<Repository<CryptoSymbol>> =
      jest.fn(() => ({
        findOne: jest.fn((querySymbol: string | any) => {
          if (querySymbol.where) {
            querySymbol = querySymbol.where.symbol;
          }
          const response = symbolList.find(
        ...
        save: jest.fn((newSymbol: CryptoSymbol) => {
          const symbolIndex = symbolList.findIndex(
            (cryptoSymbol) => cryptoSymbol.symbol == newSymbol.symbol,
          );
          if (symbolIndex == -1) {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        WatchlistService,
        {
          provide: getRepositoryToken(CryptoSymbol),
  ...
  describe('getAverageLectures method', () => {
    ...
    it('should return the correct average', async () => {
      ...
      expect(
        (await service.getAverageLectures(averageSymbol, 2)).average,
      ).toEqual(75);
    });
    it('should return the maximum number of lectures', async () => {
      expect(
        (await service.getAverageLectures(averageSymbol, 10)).numberOfLectures,
      ).toEqual(2);
    });
  });
});
